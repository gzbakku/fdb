...
pub fn init(act:&Act) -> Result<(),&'static str> {
    ...
    data.insert("item_value",act.item_value.clone()).unwrap();
    ...
    match send_to_random_member(&req,&file_anchor){
        ...
        Err(_)=>{
            return Err("failed-send_to_random_member-init-add_item");
        }
    }
}
    ...
    match block_on(send_to_member(&"warehouse".to_string(),&member_name,message,false)){
        Ok(resp)=>{
            if is_successfull(&resp.data){
                ...
                return Err("failed-process_request-send_to_this_member-init-add_item-worker");
            }
        },
        Err(_)=>{
            return Err("failed-send_request-send_to_this_member-init-add_item-worker");
        }
    }
}
        ...
        Ok(resp)=>{
            if is_successfull(&resp.data){
                match book::add(file_anchor.clone(), resp.name.clone()){
                    ...
                    Err(_)=>{
                        println!("!!! failed-add_member_name_for_file_anchor_to_book-send_to_random_member-init-add_item-worker");
                        return Ok(());
                    }
                }
            } else {
