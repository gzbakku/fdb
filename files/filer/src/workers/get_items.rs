...
pub fn init(act:&Act) -> Result<JsonValue,&'static str> {
    ...
    match book_check(&file_anchor,&act){
        ...
        Err(_)=>{
            // println!("!!! failed-book_check-init-get_items-worker=>{:?}",e);
        }
    }
    ...
    match book_check(&file_anchor,&act){
        ...
        Err(_)=>{
            return Err("failed-book_check-after_find-init-get_items-worker=>{:?}");
        }
    }
                ...
                match book::get(file_anchor.clone()){
                    Ok(member_name)=>{
                        match call_item(&act, &member_name){
                            ...
                            Err(_)=>{
                                return Err("failed-call_member_for_item-init-get_itemss-worker");
                            }
                        }
                    },
                    Err(_)=>{
                        return Err("failed-get_book_item-init-get_itemss-worker");
                    }
                }
            } else {
                return Err("not_found-get_book_item-init-get_itemss-worker");
            }
        },
        Err(_)=>{
            return Err("failed-check_book-init-get_itemss-worker");
        }
    }
}
...
fn call_item(act:&Act,member_name:&String) -> Result<JsonValue,&'static str>{
    ...
    data.insert("items",act.items.clone()).unwrap();
    ...
    match block_on(send_to_member(&"warehouse".to_string(), member_name, &req, false)){
        Ok(resp)=>{
            if !is_successfull(&resp.data) || !with_data(&resp.data){
                return Err("failed-process_request-call-init-get_itemss-worker");
            }
            return Ok(resp.data["data"].clone());
        },
        Err(_)=>{
            return Err("failed-send_request-call-init-get_itemss-worker");
        }
    }
}
